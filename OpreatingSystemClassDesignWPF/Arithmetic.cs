...
namespace OpreatingSystemClassDesignWPF
{
    public static class Arithmetic
    {
        ...
        public static bool MakeFIFO(int memoryAddress,out int blockNum)
        {
            ...
            {
                blockNum = GlobalVariable.MemoryQueue_FIFO.Where(x => x.PageNum == pageBlock.PageNum)
                        .First().PageFrameNum;
                return false;
            }
            else
            {
                //页框是否已满
                if (GlobalVariable.MemoryQueue_FIFO.Count >= GlobalVariable.MemoryBlockNum)
                {
                    //取队列第一个内存块的页框号
                    //将页框内此号码的页面数更新为此块
        ...
        public static bool MakeLRU(int memoryAddress,out int blockNum)
        {
            ...
            if (GlobalVariable.MemoryQueue_LRU.Any(x => x.PageNum == pageBlock.PageNum))
            {
                ...
                blockNum = GlobalVariable.MemoryQueue_LRU.Where(x => x.PageNum == pageBlock.PageNum)
                        .First().PageFrameNum;
                return false;
            }
            else
            {
                if (GlobalVariable.MemoryQueue_LRU.Count >= GlobalVariable.MemoryBlockNum)
                {
        ...
        public static bool MakeOPT(List<int> inputAddress, int index,out int blockNum)
        {
            ...
            {
                blockNum = GlobalVariable.MemoryQueue_OPT.Where(x => x.PageNum == pageBlock.PageNum)
                        .First().PageFrameNum;
                return false;
            }
            else
            {
                //页框是否已满
                if (GlobalVariable.MemoryQueue_OPT.Count >= GlobalVariable.MemoryBlockNum)
                {
